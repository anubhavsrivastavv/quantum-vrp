...
A, B_penalty, C_penalty, D_penalty = 100, 100, 100, 100

"""
    Checking the y_i_j constraint
"""
W_adj = np.array([[0, 10, 10], [10, 0, 10], [10, 0, 0]])
n, K = 3, 2

"""
    Checking the node with more than one visit constraint
"""
...
for v in range(1, n):
    for i in range(1, K+1):
        for k in range(K):
            for in_edge in incoming_edges(v):
                Q_Dict[('y'+str(v)+str(i), 'x'+str(k)+str(in_edge)+str(v))] = (1-2*i)*A
                Q_Dict_Track[('y'+str(v)+str(i), 'x'+str(k)+str(in_edge)+str(v))] = str((1-2*i)*A)+' (constraint-4 incoming edge('+str(v)+') linear terms for all vehicles)' 
                # Q_Dict[('y'+str(v)+str(i), 'x'+str(k)+str(in_edge)+str(v))] = -A
                # Q_Dict_Track[('y'+str(v)+str(i), 'x'+str(k)+str(in_edge)+str(v))] = str(-A)+' (constraint-4 incoming edge('+str(v)+') linear terms for all vehicles)' 

        for k, k_prime in itertools.product(range(K), range(K)):
            for in_edge in incoming_edges(v):
                for in_edge_prime in incoming_edges(v):
                    if (k, in_edge) != (k_prime, in_edge_prime):
                        Q_Dict[('y'+str(v)+str(i), 'x'+str(k)+str(in_edge)+str(v), 'x'+str(k_prime)+str(in_edge_prime)+str(v))] = 2*A
                        Q_Dict_Track[('y'+str(v)+str(i), 'x'+str(k)+str(in_edge)+str(v), 'x'+str(k_prime)+str(in_edge_prime)+str(v))] = str(2*A)+' (constraint-4 incoming edge('+str(v)+') all combination terms for all vehicles)' 

#Seems to be working fine
for v in range(1, n):
    for i in range(1, K+1):
        Q_Dict[('y'+str(v)+str(i), 'y'+str(v)+str(i))] = -A
        Q_Dict_Track[('y'+str(v)+str(i), 'y'+str(v)+str(i))] = str(-A) + 'only one y_i_v should be one'

    for i, i_prime in itertools.product(range(1, K+1), range(1, K+1)):
        if i != i_prime:
            Q_Dict[('y'+str(v)+str(i), 'y'+str(v)+str(i_prime))] = 2*A
            Q_Dict_Track[('y'+str(v)+str(i), 'y'+str(v)+str(i_prime))] = str(2*A) + 'only one y_i_v should be one'
...
# print('Offset: ', offset)
bqm = dimod.make_quadratic(Q_Dict, 5*A, dimod.BINARY)
bqm_json = json.dumps(str(bqm.adj))
# print(bqm_json)
print(bqm)
# print(bqm.adj)
...
# sampleset = dimod.ExactPolySolver().sample_hubo(Q_Dict)
